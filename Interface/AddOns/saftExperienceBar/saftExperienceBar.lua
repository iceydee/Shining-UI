...
local barHeight, barWidth = 15, 100

--Where you want the fame to be anchored
--------AnchorPoint, AnchorTo, RelativePoint, xOffset, yOffset
local Anchor = { "LEFT", ParentUI, "BOTTOMLEFT", 8, 15 }

--Fonts
showText = true -- Set to false to hide text
font,fontsize,flags = [[Interface\Addons\SharedMedia\fonts\Enigma.ttf]], 8, "MONOCHROMEOUTLINE"

--Texture used for bar
barTex = [[Interface\Addons\SharedMedia\statusbar\Armory.tga]]
...
function saftXPBar:Initialize()
	...
	backdrop:SetBackdropBorderColor(.6, .6, .6, 1)
...
function saftXPBar:ShowBar()
	if UnitLevel("player") ~= MAX_PLAYER_LEVEL then
		...
		self.frame.mouseFrame:SetScript("OnEnter", function()
			...
			GameTooltip:AddLine(string.format('Current: %s/%s (%d%%)', CommaValue(XP), CommaValue(maxXP), (XP/maxXP)*100))
