	...
	if not options then options = {
        ...
        args = {
            ...
            iconSize = {
                ...
                disabled = function()
                    local isTrue
                    if ( db.dispStyle == "Icon" ) then isTrue = false else isTrue = true end
                    return isTrue
                end,
                width = "double",
                order = 275,
...
local function buildModule(self)
    ...
    if ( not self.icon ) then self.icon = self.frame:CreateTexture(nil, "ARTWORK") end -- The icon
...
function Tracking:Refresh()
    ...
    local tracktext, icon
    
    -- To Do:
    -- Work on this: None is not showing up, which might mean it's now an active tracking type. This would
    -- explain "None" not showing when selected in-game, because technically it's "Active". 
    for id = 1, count do
    ...
    if ( SLDataText.db.profile.locked and not db.hideTooltip ) then
        ...
        self.button:SetScript("OnEnter", function(this)
            ...
            GameTooltip:AddLine(tracktext, 1, 1, 1)
            GameTooltip:Show()
        end)
    ...
    if ( db.dispStyle == L["Icon"] ) then
        self.icon:SetTexture(icon)
        if ( not self.icon:IsShown() ) then self.icon:Show() end
        if ( self.string:IsShown() ) then self.string:Hide() end
    else
        self.string:SetText(tracktext)
